...
public class DriveToDistanceCommand extends Command {
  ...
  public void execute() {
    ...
    xSpeed = Math.copySign(Math.min(Math.abs(xSpeed), DriveConstants.AUTO_MAX_SPEED_METERS_PER_SECOND), xSpeed);

    double ySpeed = driveDistanceControllerY.calculate(swerveSubsystem.getPose().getY());
    ySpeed = Math.copySign(Math.min(Math.abs(ySpeed), DriveConstants.AUTO_MAX_SPEED_METERS_PER_SECOND), ySpeed);
